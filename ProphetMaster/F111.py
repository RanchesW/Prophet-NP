...
filtered_df = df[(df['PRODNAME'] == 3300000010) & (df['TANKNUM'] == 4)].copy()
...
historical_df = filtered_df[filtered_df['ds'] < '2024-09-18']

# Prepare actual data from a specific date
actual_df = filtered_df[(filtered_df['ds'] >= '2024-09-18') & (filtered_df['ds'] <= '2024-09-19')]
...
model = Prophet(
    ...
    yearly_seasonality=False,  # Enable yearly seasonality
...
forecast_start_date = '2024-09-16 17:00:00'
forecast_end_date = '2024-09-18 23:59:59'
...
for date in forecast_dates:
    ...
    combined_forecast = (0.4 * rolling_mean_value) + (0.7 * prophet_pred)  # Weighted to favor Prophet's predictions more
...
forecast_days_df = merged_df[(merged_df.index >= '2024-09-18') & (merged_df.index < '2024-09-19')]
...
# Dead stock line
plt.axhline(y=dead_stock, color='green', linestyle='--', label='Мертвый остаток (3174 литров)')

# Actual data for 30-31 August
plt.plot(actual_df.index, actual_df['КОЛИЧЕСТВО'], label='Фактический объем за 8-10 сентября', color='green')

# Добавление легенды и заголовка
plt.legend()
plt.title('Прогноз на 8-10 сентября 2024 года с фактическими данными и гибридным методом (скользящее среднее + Prophet)')
