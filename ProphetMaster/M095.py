...
filtered_df = df[(df['PRODNAME'] == 3300000010) & (df['TANKNUM'] == 1)].copy()
...
# Reindex filtered_df to ensure all hours are present, fill missing 'КОЛИЧЕСТВО' with zeros
...
historical_df = filtered_df[filtered_df['ds'] < '2024-09-10']

# Prepare actual data from a specific date
actual_df = filtered_df[(filtered_df['ds'] >= '2024-09-09') & (filtered_df['ds'] <= '2024-09-12')]
...
forecast_start_date = '2024-09-10 15:00:00'
forecast_end_date = '2024-09-11 23:59:59'
...
forecast_days_df = merged_df[(merged_df.index >= '2024-09-09') & (merged_df.index < '2024-09-12')]
...
print(f"Средняя точность прогноза на 3-4 сентября 2024 года: {daily_accuracy:.2f}%")
